import 'package:shop/data/model/sub_category_product.dart';
...
class SubCategoryData {
  ...
  Future<List<SubCategoryProduct>> getSubCategoryListItems(
      int categoryId) async {
    final response = await supabase
        .rpc("get_category_items", params: {'category_id_param': categoryId});
    if (response.isEmpty) {
      return [];
  ...
  List<SubCategoryProduct> _getSubCategoryListItems(List<dynamic> response) {
    ...
    for (int subCategoryIndex = 0;
        subCategoryIndex < response.length;
        subCategoryIndex++) {
      ...
      List<Item> listItem =
          (itemsList as List<dynamic>).cast<Map<String, dynamic>>().map((item) {
        return Item(
          id: item['item_id'],
      ...
      subCategoryProduct.add(SubCategoryProduct(
          categoryName: subCategory['sub_category_name'], listItem: listItem));
    }
